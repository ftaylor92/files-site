...
import com.fmt.image.ImageProc;

public class UploadImage extends HttpServlet {
	private static final String THIS_URL= "http://files-site.fmtmac.cloudbees.net";
		...
		try {
			...
			while (iter.hasNext()) {
				...
				if (!item.isFormField()) {
					...
					filePath = uploadPath + File.separator + "rotated-"+ fileName;
					System.out.println("filePath: "+ filePath);
			...
			if(null != storeFile) {
				ImageProc.transform(storeFile, transFile, amount);
				storeFile.delete();
			} else {
				//TODO: err
			}
